...
async function loadAnalytics(filter) {
  try {
    ...
    analyticsBox.innerHTML =
      `Faktur: <b>${y.count ?? 0}</b> · Suma brutto: <b>${numberPL(y.total_gross)} zł</b> · VAT: <b>${numberPL(y.total_vat)} zł</b>`;
...
exportBtn.addEventListener('click', () =>
  withLoading(exportBtn, () => doExport('csv'))
);
exportJsonBtn.addEventListener('click', () =>
  withLoading(exportJsonBtn, () => doExport('json'))
);
...
(async function init() {
  ...
  await listInvoices();
  await loadAnalytics();
  document.querySelector('.footer-note')?.replaceChildren(
